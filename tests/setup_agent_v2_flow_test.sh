...
case "$method" in
  ...
  agent.models)
    ...
    printf 'models key_ok\n' >>"$FAKE_LOG"
    case "${FAKE_SCENARIO:-success}" in
      auth) jq -cn '{id:"models",error:{code:"MODEL_AUTH_FAILED",message:"authentication failed"}}' ;;
      empty) jq -cn '{id:"models",error:{code:"MODEL_CATALOG_EMPTY",message:"catalog empty"}}' ;;
      preset|override) jq -cn '{id:"models",result:{models:["preset-base","override-base"],catalog_token:"catalog-special",router_base_url:"http://127.0.0.1:19099",api_base_url:"http://127.0.0.1:19099/v1",existing:{model_config:{},unavailable_models:{},drifted_agents:[]},preset:{model_config:{version:1,claude:{primary:{model:"preset-base",name:"预设名称",context:"1m"},haiku:{inherit_primary:true},sonnet:{inherit_primary:true},opus:{inherit_primary:true}}},unavailable_agents:{}}}}' ;;
      existing) jq -cn '{id:"models",result:{models:["existing-base","preset-base"],catalog_token:"catalog-special",router_base_url:"http://127.0.0.1:19099",api_base_url:"http://127.0.0.1:19099/v1",existing:{model_config:{version:1,claude:{primary:{model:"existing-base",name:"现有名称"},haiku:{inherit_primary:true},sonnet:{inherit_primary:true},opus:{inherit_primary:true}}},unavailable_models:{},drifted_agents:[]},preset:{model_config:{version:1,claude:{primary:{model:"preset-base",context:"1m"},haiku:{inherit_primary:true},sonnet:{inherit_primary:true},opus:{inherit_primary:true}}},unavailable_agents:{}}}}' ;;
      mixed) jq -cn '{id:"models",result:{models:["existing-base","open-preset","manual-codex"],catalog_token:"catalog-special",router_base_url:"http://127.0.0.1:19099",api_base_url:"http://127.0.0.1:19099/v1",existing:{model_config:{version:1,claude:{primary:{model:"existing-base"},haiku:{inherit_primary:true},sonnet:{inherit_primary:true},opus:{inherit_primary:true}}},unavailable_models:{},drifted_agents:[]},preset:{model_config:{version:1,claude:{primary:{model:"must-not-merge",name:"preset-only-name"},haiku:{inherit_primary:true},sonnet:{inherit_primary:true},opus:{inherit_primary:true}},opencode:{default_model:"open-preset",models:{"open-preset":{name:"Open 推荐"}}}},unavailable_agents:{codex:{code:"MODEL_NOT_AVAILABLE",models:["missing-codex"]}}}}}' ;;
      unavailable) jq -cn '{id:"models",result:{models:["manual-base"],catalog_token:"catalog-special",router_base_url:"http://127.0.0.1:19099",api_base_url:"http://127.0.0.1:19099/v1",existing:{model_config:{},unavailable_models:{},drifted_agents:[]},preset:{model_config:{},unavailable_agents:{claude:{code:"MODEL_NOT_AVAILABLE",models:["missing-a","missing-z"]}}}}}' ;;
      simplify-disabled) printf '%s' "$request" | jq -e '.params.agents==["claude","opencode","codex"]' >/dev/null; jq -cn '{id:"models",result:{models:["provider/slash-model","model:two"],catalog_token:"catalog-special",router_base_url:"http://127.0.0.1:19099",api_base_url:"http://127.0.0.1:19099/v1",existing:{model_config:{},unavailable_models:{},drifted_agents:[]},preset:{model_config:{},unavailable_agents:{}}}}' ;;
      *) printf '%s' "$request" | jq -e '.params.agents==["claude","opencode","codex"]' >/dev/null; jq -cn '{id:"models",result:{models:["模型／α [x]","model:two"],catalog_token:"catalog-special",router_base_url:"http://127.0.0.1:19099",api_base_url:"http://127.0.0.1:19099/v1",existing:{model_config:{},unavailable_models:{},drifted_agents:[]},preset:{model_config:{},unavailable_agents:{}}}}' ;;
    esac
    ;;
  agent.render)
    printf '%s' "$request" | jq -c '.params.model_config' >>"$FAKE_CONFIG_LOG"
    case "${FAKE_SCENARIO:-success}" in
      success) printf '%s' "$request" | jq -e '.params.catalog_token=="catalog-special" and .params.model_config.claude.primary.model=="模型／α [x]"' >/dev/null ;;
      preset) printf '%s' "$request" | jq -e '.params.model_config.claude.primary=={model:"preset-base",name:"预设名称",context:"1m"}' >/dev/null ;;
      existing) printf '%s' "$request" | jq -e '.params.model_config.claude.primary=={model:"existing-base",name:"现有名称"} and (.params.model_config.claude.primary|has("context")|not)' >/dev/null ;;
      unavailable) printf '%s' "$request" | jq -e '.params.model_config.claude.primary=={model:"manual-base",name:"手动名称",context:"1m"}' >/dev/null ;;
      simplify-disabled) printf '%s' "$request" | jq -e '.params.model_config.claude.primary.model=="provider/slash-model" and .params.model_config.opencode.default_model=="provider/slash-model" and .params.model_config.opencode.models["provider/slash-model"].reasoning==true' >/dev/null ;;
      override) ;;
    esac
    printf 'render shape_ok\n' >>"$FAKE_LOG"
    jq -cn '{id:"render",result:{model_config:{version:1},fragments:[{agent:"claude",role:"config",path:"/tmp/special path.json",format:"json",content:"redacted dynamic fragment"}]}}'
    ;;
...
slash_config="$tmp/slash model config.json"
jq --arg model 'provider/slash-model' '.claude.primary.model=$model | .opencode.default_model=$model | .opencode.models={($model):.opencode.models["模型／α [x]"]}' "$config" >"$slash_config"
...
simplify_disabled_out="$(run_pty print simplify-disabled subcommand "$slash_config")"
simplify_disabled_lines=$'\n'"${simplify_disabled_out//$'\r'/}"$'\n'
[[ "$simplify_disabled_lines" == *$'\nMODEL provider/slash-model\n'* && "$simplify_disabled_lines" == *$'\nMODEL model:two\n'* ]] || fail 'simplify-disabled catalog output was filtered'
[[ "$simplify_disabled_out" == *'redacted dynamic fragment'* ]] || fail 'simplify-disabled slash import did not render'
