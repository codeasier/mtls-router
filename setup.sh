...
manager_secret_call() {
  ...
  printf '%s' "$info_response" | jq -e --arg protocol "$MANAGEMENT_PROTOCOL_VERSION" --arg target "$expected_target" '.id == "setup-secret-info" and .error == null and .result.deployment_id != "" and .result.target == $target and .result.management_protocol_version == $protocol' >/dev/null ||
    fail "manager protocol v4 握手失败；未接受含密钥请求。"
...
prompt_value_default() {
  ...
  if [[ -n "$default_value" ]]; then prompt_value "$label [$default_value]: "; [[ -n "$PROMPT_VALUE" ]] || PROMPT_VALUE="$default_value"
  else prompt_value "$label: "; fi
}
prompt_optional_default() {
  local label="$1" default_value="$2" suffix="留空表示未设置"
  [[ -z "$default_value" ]] || suffix="默认: ${default_value}；留空接受，输入 - 清除"
  prompt_value "$label [$suffix]: "
  if [[ -z "$PROMPT_VALUE" ]]; then PROMPT_VALUE="$default_value"; elif [[ "$PROMPT_VALUE" == - ]]; then PROMPT_VALUE=""; fi
}
prompt_context() {
  local label="$1" default_value="$2"
  prompt_value_default "$label context（standard/1m）" "${default_value:-standard}"
  [[ "$PROMPT_VALUE" == standard || "$PROMPT_VALUE" == 1m ]] || fail "$label context 只接受 standard 或 1m。"
}
prompt_json_object() {
  local default_value="${2:-}"
  prompt_optional_default "$1 JSON object" "$default_value"; [[ -z "$PROMPT_VALUE" ]] && { PROMPT_JSON=null; return; }
  PROMPT_JSON="$(printf '%s' "$PROMPT_VALUE" | jq -ce 'if type=="object" then . else error("object") end')" || fail "$1 必须是 JSON object。"
}
prompt_bool() { local default_value="${2:-}"; while true; do prompt_optional_default "$1 true/false" "$default_value"; case "$PROMPT_VALUE" in "") PROMPT_JSON=null; return;; true|false) PROMPT_JSON="$PROMPT_VALUE"; return;; esac; done; }
prompt_int() { local default_value="${2:-}"; prompt_optional_default "$1 正整数" "$default_value"; [[ -z "$PROMPT_VALUE" || "$PROMPT_VALUE" =~ ^[1-9][0-9]*$ ]] || fail "$1 必须是正整数。"; PROMPT_JSON="${PROMPT_VALUE:-null}"; }

initialize_model_defaults() {
  local models="$1" agent source missing
  INITIAL_MODEL_CONFIG="$(printf '%s' "$models" | jq -ce --argjson agents "$2" '
    (.result.existing.model_config // {}) as $existing |
    (.result.preset.model_config // {}) as $preset |
    reduce $agents[] as $agent ({version:1};
      if ($existing[$agent] != null) then . + {($agent):$existing[$agent]}
      elif ($preset[$agent] != null) then . + {($agent):$preset[$agent]}
      else . end)')" || fail "manager 返回了无效的 Agent 默认配置。"
  for agent in "${TARGETS[@]}"; do
    source="$(printf '%s' "$models" | jq -r --arg a "$agent" 'if .result.existing.model_config[$a] != null then "existing" elif .result.preset.model_config[$a] != null then "preset" else "empty" end')"
    info "CONFIG SOURCE $agent: $source"
    missing="$(printf '%s' "$models" | jq -r --arg a "$agent" '(.result.preset.unavailable_agents[$a].models // []) | join(", ")')"
    [[ -z "$missing" ]] || warn "PRESET UNAVAILABLE $agent: $missing"
  done
}

build_model_config() {
  local config='{"version":1}' agent initial model name role answer entry section ids default field context input output value extra='{}' default_value models
  for agent in "${TARGETS[@]}"; do case "$agent" in
    claude)
      initial="$(printf '%s' "$INITIAL_MODEL_CONFIG" | jq -c '.claude // {}')"
      default_value="$(printf '%s' "$initial" | jq -r '.primary.model // ""')"; prompt_value_default "Claude primary model ID（不会自动选择）" "$default_value"; model="$PROMPT_VALUE"; [[ -n "$model" ]] || fail "model ID 不能为空。"
      default_value="$(printf '%s' "$initial" | jq -r '.primary.name // ""')"; prompt_optional_default "Claude primary name" "$default_value"; name="$PROMPT_VALUE"
      default_value="$(printf '%s' "$initial" | jq -r 'if .primary.context == "1m" then "1m" else "standard" end')"; prompt_context "Claude primary" "$default_value"; context="$PROMPT_VALUE"
      section="$(jq -cn --arg m "$model" --argjson n "$(optional_string "$name")" --arg x "$context" '{primary:({model:$m}+if $n==null then {} else {name:$n} end+if $x=="1m" then {context:"1m"} else {} end)}')"
      for role in haiku sonnet opus; do
        default_value="$(printf '%s' "$initial" | jq -r --arg r "$role" 'if .[$r].inherit_primary == true then "inherit" else (.[$r].model // "") end')"; prompt_value_default "Claude ${role}：输入 inherit 或 model ID" "$default_value"; answer="$PROMPT_VALUE"; [[ -n "$answer" ]] || fail "$role 不能为空。"
        if [[ "$answer" == inherit ]]; then entry='{"inherit_primary":true}'
        else default_value="$(printf '%s' "$initial" | jq -r --arg r "$role" '.[$r].name // ""')"; prompt_optional_default "Claude $role name" "$default_value"; name="$PROMPT_VALUE"; default_value="$(printf '%s' "$initial" | jq -r --arg r "$role" 'if .[$r].context == "1m" then "1m" else "standard" end')"; prompt_context "Claude $role" "$default_value"; context="$PROMPT_VALUE"; entry="$(jq -cn --arg m "$answer" --argjson n "$(optional_string "$name")" --arg x "$context" '{model:$m}+if $n==null then {} else {name:$n} end+if $x=="1m" then {context:"1m"} else {} end')"; fi
        section="$(jq -cn --argjson s "$section" --arg r "$role" --argjson e "$entry" '$s+{($r):$e}')"
      done
      extra='{}'; for field in ANTHROPIC_CUSTOM_MODEL_OPTION_DESCRIPTION ANTHROPIC_DEFAULT_HAIKU_MODEL_DESCRIPTION ANTHROPIC_DEFAULT_SONNET_MODEL_DESCRIPTION ANTHROPIC_DEFAULT_OPUS_MODEL_DESCRIPTION; do default_value="$(printf '%s' "$initial" | jq -r --arg f "$field" '.extra[$f] // ""')"; prompt_optional_default "Claude $field" "$default_value"; [[ -z "$PROMPT_VALUE" ]] || extra="$(jq -cn --argjson e "$extra" --arg f "$field" --arg v "$PROMPT_VALUE" '$e+{($f):$v}')"; done; [[ "$extra" == '{}' ]] || section="$(jq -cn --argjson s "$section" --argjson e "$extra" '$s+{extra:$e}')"; config="$(jq -cn --argjson c "$config" --argjson s "$section" '$c+{claude:$s}')" ;;
    opencode)
      initial="$(printf '%s' "$INITIAL_MODEL_CONFIG" | jq -c '.opencode // {}')"; default_value="$(printf '%s' "$initial" | jq -r '(.models // {}) | keys | join(",")')"; prompt_value_default "opencode model IDs（逗号分隔，不会自动选择）" "$default_value"; ids="$PROMPT_VALUE"; [[ -n "$ids" ]] || fail "至少需要一个 model。"; default_value="$(printf '%s' "$initial" | jq -r '.default_model // ""')"; prompt_value_default "opencode default model ID" "$default_value"; default="$PROMPT_VALUE"; [[ -n "$default" ]] || fail "opencode default model ID 不能为空。"; models='{}'; IFS=',' read -ra values <<<"$ids"
      for model in "${values[@]}"; do model="${model// /}"; [[ -n "$model" ]] || fail "model ID 不能为空。"; entry='{}'; default_value="$(printf '%s' "$initial" | jq -r --arg m "$model" '.models[$m].name // ""')"; prompt_optional_default "opencode $model name" "$default_value"; [[ -z "$PROMPT_VALUE" ]] || entry="$(jq -cn --argjson e "$entry" --arg v "$PROMPT_VALUE" '$e+{name:$v}')"; for field in reasoning attachment tool_call temperature; do default_value="$(printf '%s' "$initial" | jq -r --arg m "$model" --arg f "$field" 'if .models[$m][$f] == null then "" else (.models[$m][$f] | tostring) end')"; prompt_bool "opencode $model $field" "$default_value"; [[ "$PROMPT_JSON" == null ]] || entry="$(jq -cn --argjson e "$entry" --arg f "$field" --argjson v "$PROMPT_JSON" '$e+{($f):$v}')"; done; default_value="$(printf '%s' "$initial" | jq -r --arg m "$model" '.models[$m].limit.context // ""')"; prompt_int "opencode $model limit.context" "$default_value"; context="$PROMPT_JSON"; default_value="$(printf '%s' "$initial" | jq -r --arg m "$model" '.models[$m].limit.input // ""')"; prompt_int "opencode $model limit.input" "$default_value"; input="$PROMPT_JSON"; default_value="$(printf '%s' "$initial" | jq -r --arg m "$model" '.models[$m].limit.output // ""')"; prompt_int "opencode $model limit.output" "$default_value"; output="$PROMPT_JSON"; if [[ "$context$input$output" != nullnullnull ]]; then [[ "$context" != null && "$output" != null ]] || fail "limit 需要 context 和 output。"; entry="$(jq -cn --argjson e "$entry" --argjson c "$context" --argjson i "$input" --argjson o "$output" '$e+{limit:({context:$c,output:$o}+if $i==null then {} else {input:$i} end)}')"; fi; default_value="$(printf '%s' "$initial" | jq -c --arg m "$model" '.models[$m].modalities // empty')"; prompt_json_object "opencode $model modalities（input/output arrays）" "$default_value"; [[ "$PROMPT_JSON" == null ]] || entry="$(jq -cn --argjson e "$entry" --argjson v "$PROMPT_JSON" '$e+{modalities:$v}')"; default_value="$(printf '%s' "$initial" | jq -r --arg m "$model" 'if .models[$m].interleaved == true then "true" else (.models[$m].interleaved.field // "") end')"; prompt_optional_default "opencode $model interleaved（true/reasoning/reasoning_content/reasoning_details）" "$default_value"; value="$PROMPT_VALUE"; [[ -z "$value" ]] || { [[ "$value" == true ]] && PROMPT_JSON=true || PROMPT_JSON="$(jq -cn --arg f "$value" '{field:$f}')"; entry="$(jq -cn --argjson e "$entry" --argjson v "$PROMPT_JSON" '$e+{interleaved:$v}')"; }; default_value="$(printf '%s' "$initial" | jq -c --arg m "$model" '.models[$m].options // empty')"; prompt_json_object "opencode $model options" "$default_value"; [[ "$PROMPT_JSON" == null ]] || entry="$(jq -cn --argjson e "$entry" --argjson v "$PROMPT_JSON" '$e+{options:$v}')"; default_value="$(printf '%s' "$initial" | jq -c --arg m "$model" '.models[$m].extra // empty')"; prompt_json_object "opencode $model extra" "$default_value"; [[ "$PROMPT_JSON" == null ]] || entry="$(jq -cn --argjson e "$entry" --argjson v "$PROMPT_JSON" '$e+{extra:$v}')"; models="$(jq -cn --argjson m "$models" --arg id "$model" --argjson e "$entry" '$m+{($id):$e}')"; done; config="$(jq -cn --argjson c "$config" --arg d "$default" --argjson m "$models" '$c+{opencode:{default_model:$d,models:$m}}')" ;;
    codex)
      initial="$(printf '%s' "$INITIAL_MODEL_CONFIG" | jq -c '.codex // {}')"; default_value="$(printf '%s' "$initial" | jq -r '.model // ""')"; prompt_value_default "Codex model ID（不会自动选择）" "$default_value"; model="$PROMPT_VALUE"; [[ -n "$model" ]] || fail "model ID 不能为空。"; section="$(jq -cn --arg m "$model" '{model:$m}')"; for field in reasoning_effort reasoning_summary verbosity; do default_value="$(printf '%s' "$initial" | jq -r --arg f "$field" '.[$f] // ""')"; prompt_optional_default "Codex $field" "$default_value"; [[ -z "$PROMPT_VALUE" ]] || section="$(jq -cn --argjson s "$section" --arg f "$field" --arg v "$PROMPT_VALUE" '$s+{($f):$v}')"; done; for field in context_window auto_compact_token_limit; do default_value="$(printf '%s' "$initial" | jq -r --arg f "$field" '.[$f] // ""')"; prompt_int "Codex $field" "$default_value"; [[ "$PROMPT_JSON" == null ]] || section="$(jq -cn --argjson s "$section" --arg f "$field" --argjson v "$PROMPT_JSON" '$s+{($f):$v}')"; done; default_value="$(printf '%s' "$initial" | jq -r '.extra.model_auto_compact_token_limit_scope // ""')"; prompt_optional_default "Codex model_auto_compact_token_limit_scope（total/body_after_prefix）" "$default_value"; [[ -z "$PROMPT_VALUE" ]] || section="$(jq -cn --argjson s "$section" --arg v "$PROMPT_VALUE" '$s+{extra:{model_auto_compact_token_limit_scope:$v}}')"; config="$(jq -cn --argjson c "$config" --argjson s "$section" '$c+{codex:$s}')" ;;
...
agent_flow() {
  ...
  if [[ -z "$filter" ]]; then [[ -t 0 ]] || fail "Agent 配置需要交互选择。非交互自动化请使用 manager protocol v4 stdin。"; printf '%s' "$detect" | jq -r '.result.agents[] | select(.detected) | "DETECTED \(.agent): \(.path)"'; prompt_value "选择 Agent（逗号分隔 claude,opencode,codex）: "; filter="$PROMPT_VALUE"; [[ -n "$filter" ]] || fail "必须显式选择至少一个 Agent。"; fi
  ...
  [[ -t 0 ]] || fail "Agent 配置需要隐藏交互输入。非交互自动化请使用 manager protocol v4 stdin：manager.info 握手后调用 agent.models、agent.render/agent.preview、agent.write；key 不得进入参数、环境或文件。MTLS_ROUTER_OPENAI_API_KEY 已移除。"
...
非交互自动化请使用 manager protocol v4 stdin：manager.info 握手后依次调用
