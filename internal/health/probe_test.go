...
func TestProbeSucceedsAgainstMTLSServer(t *testing.T) {
	server, certs := newMTLSServer(t, http.StatusNoContent, 0)
	defer server.Close()
...
func TestProbeFails(t *testing.T) {
	...
	server5xx, certs5xx := newMTLSServer(t, http.StatusServiceUnavailable, 0)
	...
	slow, slowCerts := newMTLSServer(t, http.StatusNoContent, 50*time.Millisecond)
	defer slow.Close()
...
func newMTLSServer(t *testing.T, status int, delay time.Duration) (*httptest.Server, mtlsCerts) {
	t.Helper()
