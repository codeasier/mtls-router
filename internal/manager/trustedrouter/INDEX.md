...
模型目录与用量发现所需的「通道绑定的 router 信任」：确认对面确实是本代 router，才把 API key 送出去。

## 文件

| 文件 | 职责 |
|------|------|
| `coordinator.go` | `Coordinator`、`Fetch(ctx, owner, apiKey)`、`FetchUsage(ctx, owner, period, apiKey)`、`Revalidate(ctx, owner, apiKey, binding)`；`Binding`、`Result`、`UsageResult`、`Lifecycle` 接口 |
| `channel.go` | `Channel.Fetch(...)` / `FetchUsage(...)` —— 在已确认可信的 router 上取目录或用量；`versionMatches`、`trustedStateMatches` 等信任判据 |
