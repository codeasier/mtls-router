...
func TestSelectTCP4ListenerOwnerNotFound(t *testing.T) {
	buffer := windowsTCPTable(windowsTCPRow{state: 2, localIP: [4]byte{127, 0, 0, 2}, localPort: 19099, pid: 11})

	_, err := selectTCP4ListenerOwner(buffer, []byte{127, 0, 0, 1}, 19099)
...
func TestSelectTCP4ListenerOwnerFailsClosed(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "trailing data", buffer: append(windowsTCPTable(exact), 0), ip: []byte{127, 0, 0, 1}, port: 19099},
		...
		{name: "unsupported state", buffer: windowsTCPTable(windowsTCPRow{state: 5, localIP: exact.localIP, localPort: 19099, pid: 42}), ip: []byte{127, 0, 0, 1}, port: 19099},
		...
		{name: "connected row", buffer: windowsTCPTable(windowsTCPRow{state: 2, localIP: exact.localIP, localPort: 19099, remoteIP: [4]byte{127, 0, 0, 1}, remotePort: 1, pid: 42}), ip: []byte{127, 0, 0, 1}, port: 19099},
