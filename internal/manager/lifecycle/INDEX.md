...
- 桌面所有权通过 `../state` 的锁文件与 `installation.json` 谱系表达：会话 ID 只是 epoch。跨会话 reclaim 要求 installation ID 匹配且 package generation 为与当前包精确相等的正数；installation-aware generation 0 不可 reclaim 或迁移。无 installation ID 且 generation 0 的旧 schema 仅在 protocol 1/3 祖先路径中允许显式迁移，不能 reclaim；祖先协议版本集合由 `../protocol` 的 `IsLegacyLineageVersion` 单点定义。
