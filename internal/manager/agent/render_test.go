...
func renderCatalogToken(t *testing.T, service *Service, selected []Kind, models []string, baseURL string) string {
	...
	token, err := service.signer.SignCatalog(modelconfig.CatalogClaims{Models: models, Agents: agents, Owner: "cli", RouterBaseURL: baseURL, DeploymentID: "test-deployment", ProtocolVersion: "2"})
	if err != nil {
		t.Fatal(err)
	}
	return token
