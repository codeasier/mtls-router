...
| `background.go` | `PrepareSessionLogPath(basePath, startedAt)` 按日期/启动时间预留会话文件，`RecordLatestSessionLogPath`/`LatestSessionLogPath` 持久记录并恢复最后成功启动的会话；`OpenLogFile`、`OpenBoundedLogWriter(path, maxBytes)` 负责 append-only 写入与单文件 4MB 上限 |
...
- manager 仅在进程创建成功后更新私有 `.latest-session` 指针；恢复不依赖本地墙上时钟排序，CLI 与桌面 manager 分别使用各自的日志 base。
