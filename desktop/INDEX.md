...
| `diagnostic_snapshot.rs` | 内存诊断快照与 `{data_dir}/last-diagnostics.json` 原子落盘（Unix 0600，热路径不同步父目录）；`state=degraded` 分类为 `degraded`；仅在本轮 `status_error`/`health_error` 时写入 manager 诊断环 |
...
| `scheduler.rs`           | `PollScheduler` —— 周期性 router 状态/健康轮询；推进端口释放观察；emit `router-poll-snapshot`；每次 poll 由 observer 持久化最新诊断快照；可见性感知间隔                                                                                                    |
