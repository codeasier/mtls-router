...
| `router_core/`           | 嵌入式 router-core：锁定 HTTP/1 + rustls/`ring` 栈；配置/mTLS/probe/封闭启动原因码；精确 `/version` 与 `/health`；流式反向代理与白名单访问日志；独立 runtime/thread supervisor（有界 channel、请求/关闭超时、并发上限、panic containment）。不接入当前 sidecar 运行路径                                                                 |
