...
桌面应用绝不直接与 router 通信。当前默认路径仍以长驻子进程方式拉起 `mtls-router-manager serve`，带 `--desktop-session`、`--desktop-installation`、`--package-generation`、`--parent-pid/start/executable` flag。`manager_core/` 与 `router_core/` 已实现嵌入式控制面与路由核心，但尚未接入该 sidecar 路径。
...
| `NavIcon.tsx`                                             | 主导航语义图标：路由节点、Agent 终端、密钥、用量柱、日志页、设置齿轮；不引入第三方图标库                                                                                    |
...
| 文件                     | 职责                                                                                                                                                                                                                                                                                                                                                           |
| ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `lib.rs`                 | 应用入口：插件注册、setup（sidecar 校验、manager spawn、调度器启动、托盘）、invoke handler 注册；webview 仍仅 `core:default`                                                                                                                                                                                                                                   |
...
| `manager.rs`             | `ManagerClient` + `TauriTransportFactory` —— spawn/通信；watchdog 对齐 Go deadline 再加 1 秒（含 `apikey.usage` 60+1）；secret-bearing 调用不在不确定投递后 replay                                                                                                                                                                                             |
...
| `paths.rs`               | 桌面数据目录解析（委托给 `MTLS_ROUTER_DESKTOP_DATA_DIR` 或 OS 默认），并派生 `mtls-router-logs/`、`credentials.json` 与 `last_diagnostics_path`                                                                                                                                                                                                                |
| `process_identity.rs`    | `current()` —— 捕获 PID + 启动时间 + 可执行文件用于父身份 flag                                                                                                                                                                                                                                                                                                 |
| `protocol.rs`            | manager protocol v4 线类型与契约测试：19 个方法名、稳定错误码、Request/Response、严格参数、方法 deadline、legacy lineage；不接入当前 sidecar 运行路径                                                                                                                                                                                                          |
| `redaction.rs`           | 嵌入式 manager/router/supervisor 脱敏契约：字段白名单、与 Go `sanitizeText` 对齐的文本脱敏、访问日志/诊断/panic 输出矩阵测试；不接入当前 sidecar 运行路径                                                                                                                                                                                                       |
| `manager_core/`          | 嵌入式 manager 控制面：`manager.info` 元数据与生产身份校验、`diagnostics.collect` 脱敏摘要、lifecycle 状态机（start/stop/status/health/version/logs、启动失败锁存、reclaim 错误映射）、occupant 占用检查/一次性确认终止/平台身份校验、Agent 检测/catalog/preset/渲染/事务写入/cleanup claim 与 journal 回滚、trusted-router usage/catalog 的有界 fail-closed HTTP 解析与单连接通道绑定（对照冻结 Go `internal/manager/{agent,apikeyusage,modelcatalog,trustedrouter,state,paths}`）。`legacy` 解析安装/世代身份记录（冻结 `internal/manager/testdata` protocol 1/3 golden 与 installation-aware generation 谓词），并在挂上 `LegacyRuntime` 后做一次性完整身份校验停止、等待退出与端口复核；fail-closed 矩阵覆盖缺失身份、可执行文件不匹配、PID 复用、超时、拒绝发信号与端口再占用。每个安装代际最多发一次终止信号：`legacy-migration.json`（`Paths::legacy_migration_file`）在首个信号**之前**写入 pending 记录，记录只含代际/进程实例字段与已被接受的信号列表；同代际记录为 pending 或已含信号时，存活目标一律返回 `ROUTER_ALREADY_RUNNING` 且不再发信号（崩溃、重启、重试均如此），目标已退出则只清理状态并把结果记为 stopped；记录损坏或不可写时 fail closed 不发信号。迁移失败一律带 `state_reconcile` stage，因此会被 `router.status`（`start_failed` + `stage=state_reconcile code=…`）与 `router.logs` 锁存，`recent_logs` 附带脱敏的 `legacy_migration generation=… signals=… outcome=…` 行；`diagnostics.collect` summary 在存在记录时也追加同一行。不加载 secrets/凭据，也不把 sidecar 哈希当作迁移 allowlist；未新增错误码或协议字段。默认 Manager / fixtures 仍不挂 `LegacyRuntime`，`router.migrate_legacy` 保持 `method is unavailable`。`session` 把上述服务装配到现有 `ManagerClient`/`TransportFactory`（独立 session 线程，不阻塞 Tauri runtime）：生产身份填 `desktop_pid`/manager identity，occupant 读取 desktop/CLI 状态文件保护自有 PID；`TrustedCatalog` 默认仍为空，仅在已验证 desktop session 时才构造 Coordinator，不自动 HTTP 访问真实 listener。可驱动 `router_core` supervisor，但 `lib.rs` 默认路径仍是 Go sidecar。Windows sidecar ACL 仍弱于 Go DACL，仅校验 metadata                                                                                       |
| `router_core/`           | 嵌入式 router-core：锁定 HTTP/1 + rustls/`ring` 栈；配置/mTLS/probe/封闭启动原因码；精确 `/version` 与 `/health`；流式反向代理与白名单访问日志；独立 runtime/thread supervisor（有界 channel、请求/关闭超时、并发上限、panic containment）；`parity.rs` 对照冻结 Go router 的黑盒 HTTP 契约。不接入当前 sidecar 运行路径                                                                 |
