...
| `diagnostic_snapshot.rs` | 内存诊断快照与 `{data_dir}/last-diagnostics.json` 原子落盘（Unix 0600）；observer 每次 poll 覆盖写入                                                                                                                                                       |
...
| `support_bundle.rs`      | 导出用户选定 zip：`last-diagnostics.json` + `mtls-router-logs/` 全部文件；排除 credentials、Agent 配置与备份；拒绝符号链接逃逸                                                                                                                            |
