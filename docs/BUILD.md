...
Both workflows invoke `desktop/scripts/verify-package.sh` for every one of the six packages on a native matching runner. The script rejects a host/target mismatch; unpacks the NSIS, DMG, or AppImage; checks package/version identity; fails if the package contains a `mtls-router` or `mtls-router-manager` executable; checks the desktop executable's format and architecture; checks executable permissions on macOS/Linux; constructs the packaged Tauri application to initialize its registered plugins without entering the event loop; and runs the embedded manager handshake, requiring the compiled version, deployment ID, protocol, and target. Headless Linux verification runs the initialization smoke under Xvfb. The release workflow then verifies each generated `.sha256` before publication.
