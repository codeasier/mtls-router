...
This document is for maintainers building the Tauri desktop application and running the frozen Go compatibility suite. The desktop embeds the mTLS router and the management control plane in its own process; the checked-in release workflow builds five native desktop package targets (Windows x86_64, macOS Intel/Apple Silicon, Linux x86_64/arm64) and inspects each package on a matching runner. CI spot-checks an unsigned Windows x86_64 NSIS installer and Linux arm64 AppImage. Exact stable `vX.Y.Z` tag releases publish those desktop installers with signed updater artifacts and nothing else. Windows/macOS signing and macOS notarization/stapling are conditional on complete platform credentials; Tauri updater signing is separately mandatory for stable releases. Package inspection performs an initialization-only startup smoke test but does not install, normally launch, or update the application; retain separate signing-status and successful target-runner install/launch/update evidence for every released package.

> **CLI end of life.** Releases after `v0.4.1` contain no `mtls-router` or `mtls-router-manager` binaries, no setup scripts, no CLI archives, and no service wrappers; `scripts/package-release.sh` rejects any such file. The Go sources, `scripts/build.sh`, and the setup scripts remain in the repository as the frozen reference implementation: their tests prove that the embedded Rust router and manager keep the historical HTTP, protocol v4, and state-file contracts, and the release goldens under `internal/manager/testdata` drive legacy-router migration tests. Historical CLI releases stay downloadable from their tags and are never overwritten.

## Toolchains and lockfiles

- Go: `go.mod` requires Go `1.26.2`. Go is needed for the frozen compatibility tests (`go test ./...`, the uncached native occupant tests) and for the historical `scripts/build.sh`; the desktop build itself does not invoke Go.
...
Platform builds also need the Tauri 2 operating-system prerequisites: a supported WebView and native package tools, Rust target support, and standard archive/checksum tools. Placeholder router credentials are generated by the Rust build script, so `openssl` is no longer a desktop build prerequisite. Produce and launch-test desktop packages on the target operating system rather than assuming a cross-compiled bundle is installable.

## Go checks and builds (frozen CLI reference)
...
The repository builds accept the manager-only build environment variable
`SIMPLIFY`. `scripts/build.sh` (historical CLI) and `desktop/src-tauri/build.rs`
(embedded manager) normalize it before invoking any compiler: unset, empty, or
any ASCII-case spelling of `true` becomes enabled; any ASCII-case spelling of
`false` becomes disabled. Whitespace, numbers, non-ASCII lookalikes, and every
other value fail with `invalid SIMPLIFY value` before compilation. In the Go
build the normalized value is linked only into
`github.com/codeasier/mtls-router/internal/manager/modelcatalog.Simplify`; in
the desktop it is compiled into the embedded manager as `MTLS_SIMPLIFY`. The
router never receives it. Enabled is the default and excludes valid model IDs
containing ASCII `/` from the manager catalog; disabled retains every valid ID.
This is an immutable manager build policy, not a runtime setting, configuration
preference, router option, or participant in runtime configuration precedence.
...
section. `scripts/build.sh` injects it only into
`github.com/codeasier/mtls-router/internal/manager/preset.Encoded` in the
historical manager binary; `desktop/src-tauri/build.rs` compiles it into the
embedded manager as `MTLS_AGENT_MODEL_PRESET_BASE64`. The router never receives
the value. Unset or empty means no preset. A malformed nonempty value makes the
Go manager fail startup and makes the desktop's embedded manager report
`MANAGER_INIT_FAILED` before serving any Agent method, without printing the
...
The embedded client private key is a shared credential and is extractable by anyone who receives a historical router binary or a desktop package. Release only to trusted internal users. Rotation requires a replacement release with new credential material and server-side revocation of the old credential; the desktop has no runtime credential import or component updater.
...
No sidecar preparation is needed: the Rust build script embeds placeholder router credentials automatically when `secrets/` and the credential environment variables are absent, so a fresh checkout builds and tests immediately.
...
## Embedded router and manager

The desktop executable contains the mTLS router (`desktop/src-tauri/src/router_core`) and the management control plane (`desktop/src-tauri/src/manager_core`). Tauri `bundle.externalBin` is empty and the shell plugin is not a dependency: the normal path never spawns a Go child process, and `desktop/scripts/verify-package.sh` fails if a package contains `mtls-router` or `mtls-router-manager`.

`desktop/src-tauri/build.rs` supplies everything the Go linker used to inject:

| Input | Source | Compiled as |
|---|---|---|
| Client certificate, private key, upstream CA | all three files under `secrets/` at the repository root, **or** all three of `CLIENT_CERT_PEM`, `CLIENT_KEY_PEM`, `UPSTREAM_CA_PEM`; never both; none present generates a one-day placeholder pair with `rcgen` | files under `$OUT_DIR/router-credentials/`, embedded with `include_str!` in `runtime.rs` |
| Upstream URL | `UPSTREAM_URL`, default `https://upstream.placeholder.invalid` | `MTLS_UPSTREAM_URL` |
| Version, deployment ID, protocol | `VERSION` (default: crate version), `DEPLOYMENT_ID` (default `dev`), `MANAGEMENT_PROTOCOL_VERSION` (must be `4`) | `MTLS_MANAGER_VERSION`, `MTLS_DEPLOYMENT_ID`, `MTLS_MANAGEMENT_PROTOCOL_VERSION` |
| Commit and build date | `COMMIT` / `BUILD_DATE`, else `git rev-parse --short HEAD` and the current UTC time | `MTLS_ROUTER_COMMIT`, `MTLS_ROUTER_BUILD_DATE` |
| Agent model preset, catalog policy | `AGENT_MODEL_PRESET_BASE64`, `SIMPLIFY` | `MTLS_AGENT_MODEL_PRESET_BASE64`, `MTLS_SIMPLIFY` |
...
At runtime the desktop constructs the embedded manager in-process, verifies that `manager.info` reports the compiled target, version, deployment ID, and protocol, and only then serves the desktop command API. The router runs on a dedicated supervisor thread with its own Tokio runtime, request and shutdown timeouts, a concurrency cap, and panic containment; a router failure degrades status without taking the control plane down. Router access logs are written to `<data-dir>/mtls-router-logs/<date>/<time>.log` and contain only method, path, status, bytes, and latency.

### Layered local desktop development

Choose the shortest loop for the change type. None of these commands bypass the manager handshake, credential isolation, preview/revision checks, or Agent transactional writes.
...
| React UI + local 19099 | `cd desktop && npm run dev:mock:live` | Same mock shell, but the image workbench proxies catalog/chat/generation to a router already listening on `127.0.0.1:19099`. Start that router yourself. Open `http://localhost:1420/` (not `127.0.0.1:1420`; Vite binds localhost only). Vite injects a key from the desktop `credentials.json` or `WORKBENCH_API_KEY`; you can also paste one on the API key page. This path does not use the Tauri trusted channel and is stripped from production builds. |
| Rust/Tauri | `cd desktop && npm run dev:tauri:reuse` | Runs `tauri dev` with the embedded router and manager, pinning `VITE_MOCK=false` and the development identity. There is nothing to prepare first. |
...
| Router credentials, upstream, preset, or policy | set the build inputs above, then `npm run tauri -- dev` | `build.rs` re-runs when the `secrets/` files or the listed environment variables change and re-embeds them. |
| Installer / release layout | `make desktop-package-current` | Full package path. |

A full local launch with explicit development identity:
...
- Windows packages remain unsigned when either `WINDOWS_CERTIFICATE` or `WINDOWS_CERTIFICATE_PASSWORD` is unavailable. When both are present, the workflow signs the desktop executable and NSIS installer, validates both with Authenticode, and fails if the installer contains a CLI router or manager executable.
- macOS packages remain unsigned when either `APPLE_CERTIFICATE` or `APPLE_CERTIFICATE_PASSWORD` is unavailable. When both are present, the workflow signs the application executable, application bundle, and DMG, then verifies the signatures.
...
Online updates are produced only for an exact stable `vX.Y.Z` tag. Validation dispatches, prerelease tags, and other refs keep `createUpdaterArtifacts` disabled and do not advance the channel. Stable builds embed the endpoint `https://release.codeasier.top/latest.json`, and the `latest.json` generated for each release points every platform URL at `https://release.codeasier.top/mtls-router/<tag>/`. Release assembly publishes a five-platform `latest.json` (no Windows arm64), each platform's updater artifact and `.sig`, and also mirrors the complete tag directory to `downloads.codeasier.top` with an atomically advanced `latest` symlink as a secondary distribution point; the updater feed and artifact downloads are served from `release.codeasier.top`. Windows and Linux reuse the final NSIS/AppImage package as the updater artifact; macOS updater downloads use a signed `CodeasierRouter-darwin-<arch>.app.tar.gz` that is mirrored but not attached to the GitHub Release (GitHub lists only the DMGs). Every updater asset, signature, and `latest.json` is covered by `SHA256SUMS`. Sidecar `*.sha256` files are producer preflight only and are not published; GitHub asset digests cover the attached files.
...
The release workflow invokes `desktop/scripts/verify-package.sh` for each of the five packages on a native matching runner; CI does the same for its Windows x86_64 and Linux arm64 spot checks. The script rejects a host/target mismatch; unpacks the NSIS, DMG, or AppImage; checks package/version identity; fails if the package contains a `mtls-router` or `mtls-router-manager` executable; checks the desktop executable's format and architecture; checks executable permissions on macOS/Linux; constructs the packaged Tauri application to initialize its registered plugins without entering the event loop; and runs the embedded manager handshake, requiring the compiled version, deployment ID, protocol, manager target, and rustc target triple from handshake stdout. Headless Linux verification runs the initialization smoke under Xvfb. The release workflow then verifies each generated `.sha256` during aggregation preflight before publication; those sidecar files are not attached to the GitHub Release.
...
2. Inspect package contents for exactly one desktop executable, no CLI router or manager binary, and no raw PEM/key files.
3. Confirm executable permissions on macOS/Linux and current-user installation/launch without elevation.
4. Confirm the package's `SHA256SUMS` entry matches the downloaded bytes. For GitHub assets, the release digest may be used as a second check.
5. Run `manager.info` through the desktop and request router `/version` from the running embedded router; require matching version, non-default deployment ID, and management protocol `4` across desktop, embedded manager, embedded router, and release artifact metadata. Reject every mixed protocol combination before key-bearing Agent requests.
...
7. Install and launch, validate first launch, second-instance activation, embedded manager failure behavior, tray/close/quit, default autostart, external reuse, legacy router migration from a `v0.4.1` desktop or CLI installation, unknown port conflict, Agent preview/write/rollback, logs, and uninstall preparation/cleanup.
...
The current `.github/workflows/release.yml` runs five native runners that build and inspect a Windows x86_64 NSIS installer, macOS Intel/Apple Silicon DMGs, and Linux x86_64/arm64 AppImages. There is no CLI build job and no Windows arm64 package. A manual dispatch is validation-only and may select one desktop target plus an optional HTTPS upstream override; it does not produce updater artifacts. An exact stable version tag always ignores validation overrides, waits for all five desktop jobs, verifies the five package checksums and signed updater pairs, assembles the complete mirror `SHA256SUMS` and `latest.json`, attaches the GitHub-facing installer subset (macOS DMGs only; no sidecar `*.sha256`; GitHub `SHA256SUMS` lists only attached files), mirrors the complete desktop set including macOS `.app.tar.gz` updater archives plus five signing-status files, and atomically advances the `latest` updater channel. `scripts/package-release.sh` enforces an allowlist: only `CodeasierRouter-<os>-<arch>` installers, updater `.sig` files, the macOS `.app.tar.gz` updater archives, `signing-status-*.txt`, `SHA256SUMS`, and `latest.json` may enter the mirror set, and any `mtls-router*` binary, setup script, CLI archive, service wrapper, or Windows arm64 package fails packaging. Historical releases are never overwritten.

Production desktop builds require repository secrets `CLIENT_CERT_PEM`, `CLIENT_KEY_PEM`, and `UPSTREAM_CA_PEM`, plus variables `UPSTREAM_URL` and a non-default `DEPLOYMENT_ID`; `build.rs` embeds them with `RELEASE_BUILD=1` guards. Stable desktop updater publication additionally requires `TAURI_SIGNING_PRIVATE_KEY`, `TAURI_SIGNING_PRIVATE_KEY_PASSWORD`, and `TAURI_UPDATER_PUBKEY`, plus the pinned repository variable `TAURI_UPDATER_PUBKEY_SHA256`. Optional repository variable `AGENT_MODEL_PRESET_BASE64` supplies the preset to the embedded manager of every desktop package; empty is valid and means no preset. Release preflight validates a configured value through the frozen Go manager loader before matrix builds without printing its contents, and the desktop's own loader applies the same strictness at startup. Optional repository variable `SIMPLIFY` follows the normalization rules above and defaults to enabled when unset or empty; it is normalized before matrix fan-out and compiled into every desktop package as the same canonical value. The router never receives either manager-only value. Optional platform credentials select the signed/notarized release branches described above; unlike those optional credentials, all updater-key inputs are mandatory for an exact stable tag.

Each desktop matrix producer emits code-owned protocol metadata. `scripts/package-release.sh` requires exactly one metadata file per desktop producer, every file to declare schema `1` and management protocol `4`, and every producer name to start with `desktop-`, before it assembles the release. This preflight is shared by normal and recovery publication, so a valid but mixed-protocol artifact set, or one that includes CLI producers, is not publishable.
