...
Online updates are produced only for an exact stable `vX.Y.Z` tag. Validation dispatches, prerelease tags, and other refs keep `createUpdaterArtifacts` disabled and do not advance the channel. Stable builds embed the endpoint `https://release.codeasier.top/latest.json`, and the `latest.json` generated for each release points every platform URL at `https://release.codeasier.top/mtls-router/<tag>/`. Release assembly publishes a six-platform `latest.json`, each platform's updater artifact and `.sig`, and also mirrors the tag directory to `downloads.codeasier.top` with an atomically advanced `latest` symlink as a secondary distribution point; the updater feed and artifact downloads are served from `release.codeasier.top`. Windows and Linux reuse the final NSIS/AppImage package as the updater artifact; macOS additionally publishes a signed `CodeasierRouter-darwin-<arch>.app.tar.gz`. Every updater asset, signature, and `latest.json` is covered by `SHA256SUMS`.
