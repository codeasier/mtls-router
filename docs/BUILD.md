...
4. Confirm the package's `SHA256SUMS` entry and `.sha256` file match the downloaded bytes.
...
The current `.github/workflows/release.yml` runs six native runners that build and inspect Windows x86_64/arm64 NSIS installers, macOS Intel/Apple Silicon DMGs, and Linux x86_64/arm64 AppImages. There is no CLI build job. A manual dispatch is validation-only and may select one desktop target plus an optional HTTPS upstream override; it does not produce updater artifacts. An exact stable version tag always ignores validation overrides, waits for all six desktop jobs, verifies the six package checksums and signed updater pairs, assembles one `SHA256SUMS` and `latest.json`, publishes and mirrors the desktop assets plus six signing-status files, and atomically advances the `latest` updater channel. `scripts/package-release.sh` enforces an allowlist: only `CodeasierRouter-<os>-<arch>` packages, their `.sha256` and updater `.sig` files, the macOS `.app.tar.gz` updater archives, `signing-status-*.txt`, `SHA256SUMS`, and `latest.json` may be published, and any `mtls-router*` binary, setup script, CLI archive, or service wrapper fails packaging. Historical releases are never overwritten.
