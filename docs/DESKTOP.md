...
The Agents overview always presents Claude Code, OpenCode, and Codex in separate cards. Each card reports CLI installation from command lookup independently from the configuration file's existence, writability, and configured or invalid state. Detection is metadata-only and never returns the saved API-key value. It respects `CLAUDE_CONFIG_DIR`, `OPENCODE_CONFIG`, and `CODEX_HOME`; an existing Codex home directory can locate Codex configuration but does not by itself mean that the Codex CLI command is installed.

An uninstalled CLI does not block generating its configuration when the target path is writable. A successful preinstallation write remains a success and explains that the CLI must be installed before the configuration can be used. Each card opens a workflow for exactly that one Agent; the desktop does not select or batch multiple Agents in one workflow.
...
1. Refresh the overview. Open **Merge** for one valid, writable Agent. For an invalid Agent, open **Back up and rebuild** only when detection marks it eligible; other invalid configurations must be repaired manually. CLI installation does not gate either action.
2. After the card action, Rust loads the saved global API key managed on the API Keys page and discovers the manager's authenticated, build-filtered model catalog through the trusted local router for that Agent only. The overview does not load models or use the credential. Missing or invalid credentials and authentication failures return to the overview with an action to open API Keys; model-upstream and manager failures provide a safe retry or refresh action.
