...
4. Initialize each Agent independently with `existing > preset > empty`, showing whether a section came from existing configuration or the recommended preset and listing missing base IDs for any unavailable complete preset section. Configure Agent-native choices: Claude primary/role inheritance plus optional display names and Standard/1M context, an opencode model subset/default/options, and one Codex model/options. Preset values remain editable and imply no approval. Unset optional fields remain omitted. A key-free canonical model config can be imported or exported; import replaces the complete current form rather than merging with existing or preset values.
...
For Claude, canonical configuration stores the authenticated base model ID and optional exact `context: "1m"` separately. The manager appends `[1m]` only while rendering Claude's model environment values; it does not infer 1M support or write `CLAUDE_CODE_DISABLE_1M_CONTEXT`. A runtime rejection does not cause fallback or rewrite. Preset discovery itself writes no Agent file or manager transaction state, and preset data is not stored in the desktop's secret-bearing flow.
