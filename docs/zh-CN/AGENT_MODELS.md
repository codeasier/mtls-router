...
Preview 只返回路径名称和文件/state 影响，绝不返回当前值或内容。它不会创建任何持久文件系统状态：不创建文件、目录、备份、事务 journal 或协调 lock。对于已托管状态，它只打开并校验已经存在的私有事务 lock；lock 缺失或不安全时会 fail closed，绝不补建。Cleanup 专用 HMAC token 会绑定 Agent、每个源/目标路径及 keyed revision、`replace`/`delete` 操作、精确备份要求/来源、已排序删除路径、整文件漂移标志，以及 sidecar revision/operation。已缺失的记录文件会绑定为 absent、无需备份的 delete observation。Token 有意不包含 router、目录、API key 或规范 model claims。

整文件漂移要求 `approve_managed_overwrite=true`；批准不会扩大托管路径集合。Sidecar 记录的 Agent 文件若已经不存在，会视为已清理并要求漂移批准；不会为该文件创建备份或执行修改，也不会阻止继续清理 companion 文件及事务性移除 sidecar。其他读取错误仍会 fail closed。Absent observation 会在 journal 前的完整 revision barrier、每次计划修改前（包括最终 sidecar 变更），以及 journal commit 前最后一次重新校验；文件重新出现时返回 `PREVIEW_STALE`，回滚先前修改并保留所有权，不产生孤立凭据。事务已修改的普通文件不会再与其 pre-state 比较。Write 会先为每个现存且受影响的 Agent 文件和 sidecar 创建并验证私有 sibling 备份，再记录支持删除的 journal v3，按顺序处理 Agent 文件，最后更新或删除 sidecar。所有现存 Agent 文件备份都保守标记为敏感，因为保留的任意用户字段可能含凭据；无 key 的 manager sidecar 备份不敏感。现存文件的语义 JSON/TOML 根为空时，即使没有剩余托管路径可报告，也会先备份再执行 `delete`；否则执行 `replace`。Journal v3 用不存在的 post revision 表示删除；启动恢复仍把 legacy v1/v2 journal 解码为仅 replace。Rollback 先恢复 manager state，再恢复 Agent 文件，避免所有权与文件分裂。
