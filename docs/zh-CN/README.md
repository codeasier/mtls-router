...
Tauri 桌面应用提供当前用户 router 控制、托盘操作、默认登录时启动、健康/日志视图、受 supervisor 管理的端口冲突恢复、整包在线更新，以及 Claude Code、opencode 和 Codex 的显式预览/写入流程。端口恢复会报告结构化原因和 supervisor 人工引导，绝不提权或执行停止命令，并在约 10 秒内定期采样端口，报告是否检测到重新占用。manager 控制面和 mTLS router 内嵌在桌面进程中：router 运行在独立、受监督的运行时线程上，具备请求超时、有界并发和 panic 隔离，不会拉起 `mtls-router` 或 `mtls-router-manager` 子进程。首次启动绝不会修改 Agent 文件。
