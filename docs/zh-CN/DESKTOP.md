...
- **未知端口占用者：**应用不会终止它，也不会切换到其他端口。请先处理占用 `127.0.0.1:19099` 的进程，再重试。
...
退出与关闭窗口不同。退出会停止经过验证且由桌面应用所有的 router，然后退出应用。它绝不会停止兼容的外部 router 或无法验证的进程。登录时启动会在下次用户登录后启动桌面应用，并执行同样的发现和所有权规则，而不是盲目启动第二个 router。
