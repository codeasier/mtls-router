...
2. 检查已配置的回环监听（出厂默认 `127.0.0.1:19099`）。
...
登录启动注册失败不会阻断手动启动或 router 使用。设置页面会显示可恢复的自启动诊断，成功设置该选项后清除诊断。Windows 刷新已启用注册时直接更新，不会先删除注册。
...
各平台在线安装均要求受支持的安装位置和文件系统布局。如果 updater 无法替换已安装包，请退出应用，再从可信 release 渠道手工安装完整新包。
...
- **保留端口监听覆盖：**当启动失败为 `listen_failed` / `listen_refusal=access_denied` 且已配置回环地址没有占用者时，Router 页提供显式的 `127.0.0.1:<port>` 覆盖（`1–65535`）。应用绝不会自行选端口。写入 `{data_dir}/listen-override.json` 后，该地址成为本安装下次启动的唯一监听，不会先试 `19099`。重置则回到 `127.0.0.1:19099`。拒绝局域网、`0.0.0.0`、`localhost` 和 `[::1]`。覆盖后仍指向旧 URL 的 Agent 文件视为 drift，需要重新预览并写入。发现、占用恢复和 CLI 复用只看已配置监听；之后绑到 `127.0.0.1:19099` 的进程在重置前不可见。若 `listen-override.json` 损坏或不兼容，启动会 fail closed，Router 页可删除该文件并重启；见[故障排除](TROUBLESHOOTING.md)。
...
- **Windows 仅 PID 例外：**如果进程 SID 可读且与桌面用户不同，Windows 会以 `different_user` 拒绝，绝不会把该已知身份降级为仅 PID。SID 或完整进程身份无法读取时可以考虑仅 PID，但前提是 TCP4 owner table 为精确的已配置 `127.0.0.1:<port>` listener 找到唯一 PID、已排除受保护生命周期目标，并且无副作用的 `PROCESS_TERMINATE` 权限预检成功。面板只显示 PID，并警告身份、所有者、启动时间和可执行文件仍未验证。终止前一刻，manager 会重复精确 owner 和保护检查并重新取得终止权限；listener 消失、变化、重复、wildcard、格式错误、不明确或此时无权访问都会被拒绝且不发送信号。无法读取的生命周期状态和 PID 重用仍是残余风险。应用绝不请求提权。
...
API 密钥页面会在桌面凭据存储中保存、替换或删除一个全局 API key；webview 只能读取摘要，绝不能回读明文。删除已保存密钥前会先要求确认。主导航中独立的「用量」页面展示当前密钥的用量窗口：周期选择按滚动（`1h` / `12h` / `24h` / `7d` / `30d`）与日历（`today` / `this_week` / `this_month`）分组，按模型明细表支持模型多选筛选，以及请求、Token、费用列排序。日历文案写明北京时间 08:00 起点（与上游 UTC 日 / ISO 周 / 月边界对齐）；桌面只转发 `period` token，不计算窗口，也不从 `quotas[]` 合成总览配额。Rust 一次性加载 key，manager 经可信本地路由查询 `GET /v1/usage`，webview 只收到有界的 per-key 快照（请求数、token、费用、可选总览配额、可选供应商 `quotas[]` 与按模型明细）。快照 `as_of` 与重置时间均按 `Asia/Shanghai` 格式化。若服务尚未提供该接口，窗口只报告用量不可用，不泄露 key 或上游细节。Agent 总览既不读取也不验证该 key。只有点击卡片操作后，Rust 才会按需为 `agent.models` 加载它，并在 `agent.write` 前再次加载当前保存值。桌面 `ModelFlow` 只包含 Agent、目录和预览模式状态，不包含 API key。Timeout、malformed response、manager restart 或 uncertain delivery 后，secret-bearing 调用绝不会自动 replay。除凭据存储以及 Agent 文件或已批准备份外，应用不会有意把 key 放入桌面或 manager 持久状态、进程参数、环境变量、日志、诊断、model config、catalog/revision token、预览响应或写入响应。
...
普通卸载保留应用数据。Windows 明确勾选**删除应用数据**时，会清理私有业务目录 `%APPDATA%\com.codeasier.mtls-router`，包括已保存凭据、工作台会话和图片、manager 状态及诊断，同时清理 Tauri 自身的数据目录。更新、未勾选或静默卸载均保留这些目录。外部 Agent 配置及其相邻的敏感备份始终保留，卸载不会恢复或重写它们。选择删除或单独移除保留文件前，请先确认恢复和诊断保留需求。
