...
3. Read the structured diagnostic. `force_terminate` means the target is eligible for explicit confirmation. `manual_stop_required` means a service, insufficient privilege, or another user must be handled outside the app. `unavailable` means the target is protected or its identity cannot be established safely. Blocked diagnostics contain no confirmation token.
