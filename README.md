...
The Tauri desktop application provides current-user router control, tray operation, default launch-at-login, health/log views, supervised port-conflict recovery, whole-package online updates, and explicit preview/write flows for Claude Code, opencode, and Codex. Port recovery reports structured reasons and supervisor guidance, never elevates or executes stop commands, and periodically samples the port for about 10 seconds to report whether reoccupation was detected. The manager control plane and the mTLS router are embedded in the desktop process: the router runs on a dedicated supervised runtime thread with request timeouts, bounded concurrency, and panic containment, and no `mtls-router` or `mtls-router-manager` child process is spawned. The desktop never changes Agent files on first launch.
