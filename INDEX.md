...
- `scripts/package-release.sh` 对精确 stable tag 汇总五平台 updater artifact/signature、生成 `latest.json`（平台 URL 指向 `release.codeasier.top`，darwin 仍指向 `.app.tar.gz`）并纳入镜像 `SHA256SUMS`；GitHub Release 只挂五个安装包（macOS 仅 DMG）、Windows/Linux 安装包对应的 updater `.sig`、`latest.json`、只含已挂文件的 `SHA256SUMS` 与五个 `signing-status`。macOS `.app.tar.gz` updater 归档及其 `.sig` 只镜像到 `release.codeasier.top`，完整校验清单也只在镜像 `SHA256SUMS`。release/recovery workflow 另将完整 tag 目录镜像至 `downloads.codeasier.top` 并单调、原子推进其 `latest` symlink 作为二级分发点，而 updater feed 与产物下载均以 `release.codeasier.top` 为准。Updater 签名密钥与 Windows/macOS 平台签名凭据属于独立信任链。不发布 sidecar `*.sha256`。
